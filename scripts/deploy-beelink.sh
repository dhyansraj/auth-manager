...
ssh "$BUILD_HOST" "cd '$REPO_REMOTE_PATH' && \
  docker build -f dev/openresty/Dockerfile -t '$REGISTRY/platform-edge:0.1.4' . && \
  docker push '$REGISTRY/platform-edge:0.1.4'"
ok "platform-edge:0.1.4 pushed to $REGISTRY"
