...
#   - DB_PASSWORD + KC_ADMIN_PASSWORD + PG_SUPERUSER_PASSWORD env vars set
#     (dev creds, NOT prod's)
#
...
# CNPG `postgres` superuser password. We pre-create the superuser secret
# ourselves (basic-auth type) and pin it via enableSuperuserAccess: true, so
# CNPG syncs the role password to this value instead of drifting (#92).
...
if [ -z "$PG_SUPERUSER_PASSWORD" ] || [ "$PG_SUPERUSER_PASSWORD" = "postgres" ]; then
  echo "ERROR: PG_SUPERUSER_PASSWORD env var must be set to a non-default value." >&2
  echo "  Example: export PG_SUPERUSER_PASSWORD='<dev-value-from-password-manager>'" >&2
  exit 2
fi
...
# storageClass as prod (longhorn). The CNPG operator creates the rw/ro/r
# services automatically.
#
# Both credential secrets are pre-created by us (NOT auto-generated by CNPG):
#   - platform-pg-dev-app-secret       → bootstrap.initdb owner creds
#   - platform-pg-dev-superuser-secret → `postgres` superuser, basic-auth type
# Pinning the superuser secret + enableSuperuserAccess: true keeps CNPG from
# drifting the `postgres` password on each reconcile (#92).
if $DEBUG_MODE; then
  echo "  (would create platform-pg-dev-app-secret + platform-pg-dev-superuser-secret + apply CNPG Cluster manifest)"
else
  ...
  # Superuser secret MUST be type kubernetes.io/basic-auth for CNPG to adopt
  # it as a user-supplied superuser secret (keys username + password).
